...
namespace UnitTestTwitterViaTweetinviConsole
{
    class Program
    {
        static void Main()
        {
            ...
            var OAuthToken = ConfigurationManager.AppSettings["TwitterOAuthToken"];
            var OAuthTokenSecret = ConfigurationManager.AppSettings["TwitterOAuthTokenSecret"];
            var ConsumerKey = ConfigurationManager.AppSettings["TwitterConsumerKey"];
            var ConsumerSharedSecret = ConfigurationManager.AppSettings["TwitterConsumerSharedSecret"];
            ...
            var tweet = Tweet.PublishTweet("@RPeplau hope you are having a good year so far. Thanks @TweetinviApi you are awesome.");
            if (tweet.IsTweetPublished == false) throw new Exception("ERROR Publishing text only tweet");

            // Tweet with image
            byte[] file1 = System.IO.File.ReadAllBytes("C:\\Users\\chris\\Pictures\\DIuqzTxUEAAD9ME.jpg");
            tweet = Tweet.PublishTweetWithImage("@RPeplau This was the beard last year (Tweetsharp) I trimmed it to use @TweetinviApi which is much better", file1);
